...
object AppPrompterEngine {
  ...
  "features": [${spec.features.joinToString(",") { "\"" + escapeJson(it) + "\"" }}]
        ...
        if (lowerPrompt.contains("startseite") || lowerPrompt.contains("home")) {
            files["screens/home.html"] = screenHtml(spec.appName, "Startseite", "Willkommen in deiner App.")
        }
        if (lowerPrompt.contains("notiz")) {
            files["screens/notes.html"] = screenHtml(spec.appName, "Notizen", "Notizen können hier organisiert werden.")
        }
        if (lowerPrompt.contains("einstellung")) {
            files["screens/settings.html"] = screenHtml(spec.appName, "Einstellungen", "App-Einstellungen.")
        }
        return GeneratedAppProject(spec, files)
