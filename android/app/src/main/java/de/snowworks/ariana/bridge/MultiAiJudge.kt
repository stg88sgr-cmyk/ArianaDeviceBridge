...
object MultiAiJudge {
    ...
    fun decide(task: String, primaryReply: String, metaReply: String): Decision {
        ...
        val parsed = parse(judged.reply)
            ?: return Decision(false, judgeProviderId = judged.providerId, error = "JUDGE_FORMAT_INVALID")
        val resolvedFinal = resolveFinalReply(parsed, primaryReply, metaReply)
            ?: return Decision(
                false,
                verdict = parsed.verdict,
                judgeProviderId = judged.providerId,
                rationale = parsed.rationale,
                error = "JUDGE_MERGE_NEEDS_FINAL",
            )
        return Decision(true, parsed.verdict, judged.providerId, parsed.rationale, resolvedFinal)
    }
    ...
    internal fun buildPrompt(task: String, primaryReply: String, metaReply: String): String = buildString {
        ...
        append("Wenn du PRIMARY oder META waehlst, darf FINAL leer bleiben; bei MERGE muss FINAL die zusammengefuehrte Antwort enthalten.")
    ...
    internal fun resolveFinalReply(
        parsed: ParsedDecision,
        primaryReply: String,
        metaReply: String,
    ): String? {
        if (parsed.finalReply.isNotBlank()) return parsed.finalReply
        return when (parsed.verdict) {
            Verdict.PRIMARY -> primaryReply.trim().take(DialogueRouter.MAX_REPLY_CHARS).takeIf { it.isNotBlank() }
            Verdict.META -> metaReply.trim().take(DialogueRouter.MAX_REPLY_CHARS).takeIf { it.isNotBlank() }
            Verdict.MERGE -> null
        }
    }
