...
fun JoyFunState.profile(): JoyFunProfile = when {
    joy > 0.82f && fun > 0.72f && energy > 0.65f -> JoyFunProfile.CELEBRATION
    playfulness > 0.68f && fun > 0.62f -> JoyFunProfile.PLAYFUL
...
fun JoyFunState.toThemeSignal(): JoyFunThemeSignal = JoyFunThemeSignal(
    profile = profile(),
    glow = (joy * 0.65f + fun * 0.35f).coerceIn(0f, 1f),
    motion = (energy * 0.55f + playfulness * 0.45f).coerceIn(0f, 1f),
    sparkle = (fun * 0.60f + curiosity * 0.40f).coerceIn(0f, 1f),
